...
def authenticate_company(db: Session, company_username_or_email: str, company_password: str) -> User:
    """
    Authenticate a company and return a virtual COMPANY role user
    Now supports login with either company_username OR company_email
    """
    print(f"[DEBUG] Company login attempt - Username/Email: {company_username_or_email}")
    
    # Find company by username OR email
    company = db.query(Company).filter(
        (Company.company_username == company_username_or_email) | 
        (Company.company_email == company_username_or_email)
    ).first()
    
    if not company:
        print(f"[DEBUG] Company not found with username/email: {company_username_or_email}")
        # List all companies for debugging
        all_companies = db.query(Company).all()
        print(f"[DEBUG] Available companies: {[(c.id, c.name, c.company_username, c.company_email) for c in all_companies]}")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid company username/email or password."
        )
    
    print(f"[DEBUG] Company found: ID={company.id}, Name={company.name}, Username={company.company_username}, Email={company.company_email}, Active={company.is_active}")
