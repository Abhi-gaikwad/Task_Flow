...
async def unified_login(
    ...
    try:
        ...
        try:
            company_user = authenticate_company(db, form_data.username, form_data.password)
            if company_user:
                print(f"[DEBUG] Company login successful: {company_user.username}")
            ...
            if he.status_code != status.HTTP_401_UNAUTHORIZED:
                print(f"[DEBUG] Company login failed with non-401 error: {he.detail}")
                raise
        ...
        print(f"[DEBUG] Unexpected error during unified login: {type(e).__name__}: {str(e)}")
        import traceback
