...
export const Dashboard: React.FC = () => {
  ...
  // Load companies after users are loaded
  useEffect(() => {
  ...
  const shouldShowUserSection = user?.role === 'admin' || user?.role === 'super_admin';
  ...
  return (
    <div className="space-y-6">
                ...
                <button className="w-full text-left px-4 py-3 bg-green-50 hover:bg-green-100 rounded-lg transition-colors">
                  <div className="flex items-center">
                    ...
                    <span className="text-green-900 font-medium">
                      {user?.role === 'super_admin' ? 'Add New Company & Admin' : 'Add New User'}
                    </span>
                  </div>
                </button>
              )}
      ...
      {/* Super Admin: Simple Companies List */}
      ...
      {/* User Management Section - Shows Company Admins for Super Admin */}
      {shouldShowUserSection && (
        <div className="bg-white rounded-lg shadow">
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-gray-900 flex items-center">
                ...
                {user?.role === 'super_admin' ? 'Company Administrators' : 'Team Members'}
              </h2>
              <div className="flex items-center space-x-2">
            ...
            {usersLoading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">
                  {user?.role === 'super_admin' ? 'No company administrators found' : 'No users found'}
                </p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200 text-left">
                      ...
                      {user?.role === 'super_admin' && <th className="pb-3 text-sm font-medium text-gray-600">Company</th>}
                  ...
                  <tbody>
                    {users
                      .filter(dashboardUser => 
                        user?.role === 'super_admin' 
                          ? true // Super admin sees all users (company admins)
                          : dashboardUser.company?.id === user?.company_id // Admin sees only their company users
                      )
                      .slice(0, 10)
                      .map((dashboardUser) => (
                        ...
                        {user?.role === 'super_admin' && (
                          <td className="py-3">
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="text-sm text-gray-700">{dashboardUser.company?.name || 'N/A'}</span>
                            </div>
                          </td>
                        )}
                ...
                {users.filter(u => user?.role === 'super_admin' ? true : u.company?.id === user?.company_id).length > 10 && (
                  <div className="mt-4 text-center">
                    <button className="px-4 py-2 text-sm text-blue-600 hover:text-blue-800 font-medium">
                      View all {users.filter(u => user?.role === 'super_admin' ? true : u.company?.id === user?.company_id).length} users →
                    </button>
                  </div>
                )}
              </div>
